       let cart = [];
        ...
        function renderCart() {
            ...
            const mobileTbody = $("#mobileCartList");
            const cartSection = $(".desktop-cart");
            tbody.empty();
            mobileTbody.empty();
            let subtotal = 0;
            ...
            if (cart.length === 0) {
                ...
                cart.forEach((item, index) => {
                    ...
                    mobileTbody.append(`
                        <tr>
                            <td>${item.nama}</td>
                            <td class="text-center">
                                <button class="btn btn-sm btn-outline-secondary mobileMinusBtn" data-index="${index}">-</button>
                                <span class="mx-2">${item.qty}</span>
                                <button class="btn btn-sm btn-outline-secondary mobilePlusBtn" data-index="${index}">+</button>
                            </td>
                            <td class="text-end">Rp${total.toLocaleString("id-ID")}</td>
                            <td>
                                <button class="btn btn-sm btn-danger mobileRemoveBtn" data-index="${index}">
                                    <i class="bi bi-trash"></i>
                                </button>
                            </td>
                        </tr>
                ...
                // Update mobile cart
                $("#mobileCartSubtotal").text(subtotal.toLocaleString("id-ID"));
                $("#mobileCartDiscount").text(discountAmount.toLocaleString("id-ID"));
                $("#mobileCartTax").text(taxAmount.toLocaleString("id-ID"));
                $("#mobileCartTotal").text(totalAfter.toLocaleString("id-ID"));
            }

            const totalItems = cart.reduce((a, b) => a + b.qty, 0);
            $("#cartCount").text(totalItems);
            $("#mobileCartCount").text(totalItems);
            $("#mobileCartItemCount").text(totalItems + " item");
        }

        $(document).on("click", ".plusBtn, .mobilePlusBtn", function () {
            const index = $(this).data("index");
            cart[index].qty++;
            renderCart();
        });

        $(document).on("click", ".minusBtn, .mobileMinusBtn", function () {
            const index = $(this).data("index");
            if (cart[index].qty > 1) {
                cart[index].qty--;
            } else {
        ...
        $(document).on("click", ".removeBtn, .mobileRemoveBtn", function () {
            const index = $(this).data("index");
            cart.splice(index, 1);
            renderCart();
        });

        $("#payBtn, #mobilePayBtn").on("click", function () {
            if (cart.length === 0) {
        ...
        $("#searchMenu").on("input", function () {
            ...
            $("#menuList .menu-horizontal-item").each(function () {
                const nama = $(this).find(".card-title").text().toLowerCase();
                $(this).toggle(nama.includes(keyword));
            });
        });

        $("#discount, #tax, #mobileDiscount, #mobileTax").on("input", function () {
            // Sync values between desktop and mobile inputs
            if (this.id === "discount") {
                $("#mobileDiscount").val($(this).val());
            } else if (this.id === "tax") {
                $("#mobileTax").val($(this).val());
            } else if (this.id === "mobileDiscount") {
                $("#discount").val($(this).val());
            } else if (this.id === "mobileTax") {
                $("#tax").val($(this).val());
            }
            renderCart();
        });
        ...
        $("#mobileCartToggle").on("click", function() {
            $("#mobileCart").toggleClass("hidden");
        });
        
        $("#mobileCartHeader").on("click", function() {
            $("#mobileCart").addClass("hidden");
        });
