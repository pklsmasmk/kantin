        ...
        <main>
            ...
            <nav class="tabs" role="tablist" aria-label="Navigasi Shift">
                <button role="tab" aria-selected="<?= $active_tab === 'current' ? 'true' : 'false' ?>"
                    class="<?= $active_tab === 'current' ? 'active' : '' ?>" tabindex="0"
                    data-tab="current">Cashdrawer</button>
                <button role="tab" aria-selected="<?= $active_tab === 'setoran' ? 'true' : 'false' ?>"
                    class="<?= $active_tab === 'setoran' ? 'active' : '' ?>" tabindex="-1"
                    data-tab="setoran">Setoran</button>
                <button role="tab" aria-selected="<?= $active_tab === 'history' ? 'true' : 'false' ?>"
                    class="<?= $active_tab === 'history' ? 'active' : '' ?>" tabindex="-1"
                    data-tab="history">Riwayat</button>
            </nav>
            ...
            <section class="tab-panel current-panel <?= $active_tab === 'current' ? 'is-active' : '' ?>"
                ...
                <section class="info">
                    ...
                    <div style="margin-top: 12px; padding-left: 16px; border-left: 3px solid #8B7355;">
                        ...
                        <ul style="margin: 8px 0; padding-left: 20px; color: #555;">
                            <li><strong>Mulai Shift</strong> - Input saldo awal, sistem otomatis tambah saldo warisan
                            </li>
                            <li><strong>Operasional</strong> - Transaksi penjualan, pengeluaran, pemasukan/pengeluaran
                                lain</li>
                            <li><strong>Setoran Fleksibel</strong> - Setor kapan saja dari saldo akhir yang tersedia
                            </li>
                            <li><strong>Rekap Detail</strong> - Monitoring lengkap transaksi dan saldo</li>
                            <li><strong>Akhiri Shift</strong> - Sistem hitung otomatis, saldo akhir jadi warisan
                                berikutnya</li>
                        </ul>

                        <div style="margin-top: 10px; padding: 8px; background: #fff3cd; border-radius: 6px;">
                            <small><strong>Saldo Warisan:</strong> Saldo akhir shift sebelumnya otomatis menjadi bagian
                                saldo awal shift baru</small>
                        </div>
                    </div>
                </section>
                ...
                <?php if ($saldo_warisan > 0): ?>
                    <div class="saldo-warisan-info">
                        <small>Saldo warisan dari shift sebelumnya: <strong><?= format_rupiah($saldo_warisan) ?></strong>
                            akan ditambahkan ke saldo awal</small>
                    </div>
                <?php endif; ?>
                ...
                <form method="POST" class="shift-form" novalidate>
                    ...
                    <div class="refresh-wrapper">
                        ...
                        <button type="button" class="refresh-btn" title="Refresh cashdrawer"
                            id="refreshCashdrawer">&#x21bb;</button>
                    </div>

                    <label for="saldo_awal">Masukkan Saldo Awal</label>
                    <div class="input-rp">
                        <span class="rp-label">Rp</span>
                        <input type="text" id="saldo_awal" name="saldo_awal" placeholder="0" autocomplete="off"
                            inputmode="numeric" pattern="[0-9.,]*" required value="" />
                    </div>
                    <small id="lastData" class="last-data">
                        <?php if (!empty($history)): ?>
                            Data terakhir: <?= htmlspecialchars($history[0]["cashdrawer"]) ?> •
                            Saldo awal: <?= format_rupiah($history[0]["saldo_awal"]) ?> • Saldo akhir:
                            <?= format_rupiah($history[0]["saldo_akhir"]) ?> •
                    ...
                    <?php if ($saldo_warisan > 0): ?>
                        <div class="warisan-notice">
                            <small><strong>SALDO WARISAN:</strong> <?= format_rupiah($saldo_warisan) ?> akan ditambahkan ke
                                saldo awal Anda</small>
                        </div>
                    <?php endif; ?>
                    ...
                    <div class="action-buttons">
                        <a href="/shift/Rekap_Shift/rekap_shift.php" class="action-btn manage-cash-btn"
                            <?= !$currentShift ? 'style="opacity: 0.6; pointer-events: none;"' : '' ?>>
                ...
                <div class="konten-modal-konfirmasi">
                    ...
                    <div class="body-modal-konfirmasi">
                        ...
                        <div class="catatan-warisan">
                            <small><strong>PERHATIAN:</strong> Setelah shift dimulai, data akan tersimpan di sistem dan
                                masuk ke riwayat shift.</small>
                        </div>
                    </div>
            ...
            <section class="tab-panel setoran-panel <?= $active_tab === 'setoran' ? 'is-active' : '' ?>"
                data-tab-panel="setoran" aria-label="Manajemen setoran kas">
                ...
                <div class="shift-status-info">
                    ...
                    <?php else: ?>
                        <div class="status-inactive">
                            <span
                                class="status-badge"><?= $saldo_akhir_riwayat > 0 ? '🟡 Saldo Tersedia' : '🔴 Tidak Ada Saldo' ?></span>
                            <small>
                    ...
                    <form method="POST" class="setoran-form" novalidate>
                        ...
                        <div class="form-grid">
                            <div class="form-group">
                                ...
                                <input type="text" id="penyetor" name="penyetor" required
                                    ...
                                    value="<?= htmlspecialchars($currentShift['nama'] ?? 'User Kasir') ?>">
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <?php if ($can_setor): ?>
                                    <small class="saldo-tersedia">Saldo tersedia untuk disetor:
                                        <?= format_rupiah($saldo_tersedia) ?></small>
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        <?php if (!$can_setor && $saldo_akhir_riwayat > 0): ?>
                            <div class="isi-ulang-info">
                                <small><strong>Tidak ada saldo tersisa untuk disetor. </strong> Silakan mulai shift baru
                                    untuk menambah saldo.</small>
                            </div>
                        ...
                        <div class="info-item">
                            ...
                            <span class="info-value">
                                <?php if ($saldo_tersedia > 0): ?>
                                    <span style="color: #28a745;">● Masih bisa setor (<?= format_rupiah($saldo_tersedia) ?>
                                        tersedia)</span>
                                <?php else: ?>
            ...
            <section class="tab-panel history-panel <?= $active_tab === 'history' ? 'is-active' : '' ?>"
                data-tab-panel="history" aria-label="Riwayat shift cashdrawer">
                        ...
                        <div class="search-input-group">
                            ...
                            <div class="date-input-wrapper">
                                ...
                                <?php if (isset($_GET['search_date']) && $_GET['search_date'] !== date('Y-m-d')): ?>
                                    <a href="<?= $_SERVER['PHP_SELF'] ?>?tab=history" class="clear-btn">Tampilkan Hari
                                        Ini</a>
                                <?php endif; ?>
                            </div>
                        </div>
                ...
                <?php else: ?>
                    ...
                    <ul class="history-list compact-view">
                        <?php foreach ($displayHistory as $item):
                            ...
                            <li class="history-card compact-card" data-shift-id="<?= $item['id'] ?>"
                                ...
                                <div class="compact-user">
                                    <small><?= htmlspecialchars($item["nama"]) ?> •
                                        <?= htmlspecialchars($item["role"]) ?></small>
                                </div>
